...
## T-071 — API e Hooks de Categorias [pendente]

- Refs: US-026, AC-118
- Arquivos: src/services/categories.service.ts, src/hooks/useCategories.ts
- Notas: GET/POST/PATCH/DELETE na rota `/api/categorie`. Importante usar `display_id`.

## T-072 — UI de Gerenciamento de Categorias [pendente]
...
## T-073 — API e Hooks de Contrapartes [pendente]

- Refs: US-027, AC-118
- Arquivos: src/services/counterparties.service.ts, src/hooks/useCounterparties.ts
- Notas: Chamadas para `/api/counterpartie`. `display_id` no path params das exclusões e edições.

## T-074 — UI de Gerenciamento de Contrapartes [pendente]
...
## T-075 — API e Hooks de Métodos de Pagamento [pendente]

- Refs: US-028, AC-118
- Arquivos: src/services/pay-methods.service.ts, src/hooks/usePayMethods.ts
- Notas: Endpoint `/api/pay-method`.

## T-076 — UI de Métodos de Pagamento [pendente]
...
## T-077 — API de Gerenciamento de Carteira [pendente]
...
## T-078 — UI de Configurações da Carteira [pendente]
...
## T-079 — UI de Placeholder para Equipe [pendente]

- Refs: US-030, AC-116
- Arquivos: src/pages/Settings/MembersSettings.tsx
- Notas: Um card bonito explicando que gestão de times virá em breve.

## T-080 — Layout da Tela de Configurações [pendente]

- Refs: US-030, AC-117
- Arquivos: src/pages/Settings/index.tsx, src/pages/Settings/SettingsSidebar.tsx
- Notas: Criar um layout com menu lateral (sidebar responsivo) usando React Router Outlet.

## T-081 — Schemas de Validação [pendente]

- Refs: US-026, US-027, US-028, US-029
- Arquivos: src/pages/Settings/settingsSchemas.ts
- Notas: Centralizar validação zod para todos os modais (Categorias, Métodos de pagamento, Contrapartes, Wallet).

## T-082 — Componentes Compartilhados de Configurações [pendente]

- Refs: US-026, US-027, US-028
- Arquivos: src/pages/Settings/SettingsListLayout.tsx, src/pages/Settings/ActionMenu.tsx
- Notas: Layout genérico para não repetir o esqueleto de "Header, Botão Adicionar, Lista com menu 3 pontos".
